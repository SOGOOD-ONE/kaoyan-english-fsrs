...
def card_json(card):
    return {"id": card.id, "wordId": card.word_id, "state": card.state, "stability": card.stability,
            ...
            "reps": card.review_count, "lapses": card.wrong_count}
...
@api.post("/auth/login")
@login_required
def login_required_placeholder(user): return jsonify({"error": "internal"}), 500
...
def create_review(user):
    ...
    previous_review_count = card.review_count; card.review_count = int(card_payload.get("reviewCount", previous_review_count + 1)); card.correct_count = int(card_payload.get("correctCount", card.correct_count + (0 if rating == 1 else 1))); card.wrong_count = int(card_payload.get("wrongCount", card.wrong_count + (1 if rating == 1 else 0)); card.state = str(card_payload.get("state", card.state)); card.stability = float(card_payload.get("stability", card.stability)); card.difficulty = float(card_payload.get("difficulty", card.difficulty)); card.due_at = parse_client_datetime(card_payload.get("dueAt"), reviewed_at); card.last_review_at = reviewed_at
