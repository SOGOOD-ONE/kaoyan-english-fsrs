...
def register():
    ...
    session.clear(); session["user_id"] = user.id; return jsonify({"user": user_json(user)}), 201
...
def login():
    ...
    user.last_login_at = datetime.utcnow(); db.session.commit(); session.clear(); session["user_id"] = user.id
...
@login_required
def words(user):
    """Search public words; optional filters: category, source, vocabularyId, q."""
    query = Word.query
    args = request.args
    ...
    if args.get("vocabularyId"):
        query = query.join(VocabularyWord, VocabularyWord.word_id == Word.id).filter(VocabularyWord.vocabulary_id == args["vocabularyId"])
    rows = query.order_by(Word.category.asc(), Word.created_at.asc()).limit(min(int(args.get("limit", 100)), 500)).all()
    return jsonify([{"id": w.id, "word": w.word, "type": w.word_type, "meaning": w.meaning, "category": w.category,
                     "source": w.source, "sourceDetail": w.source_detail} for w in rows])


@api.get("/study/today")
@login_required
def study_today(user):
    today = date.today(); settings = UserSetting.query.filter_by(user_id=user.id).first() or UserSetting(user_id=user.id)
    ...
    # New-word ranking: system priority > category difficulty > older untouched words.
    category_weight = {"核心词": 100, "长难词": 95, "难词": 90, "短语": 85, "固定搭配": 85}
    candidates = Word.query.filter(~Word.id.in_(active_words)).all() if active_words else Word.query.all()
    ...
    return jsonify({"date": today.isoformat(), "review": [card_json(c) for c in due],
                    "newWords": [{"id": w.id, "word": w.word, "type": w.word_type, "meaning": w.meaning, "category": w.category, "source": w.source} for w in new_words],
                    "newTotal": len(new_words), "newCompleted": plan.new_completed, "reviewTotal": len(due),
                    "reviewCompleted": ReviewLog.query.filter(ReviewLog.user_id == user.id, ReviewLog.reviewed_at >= datetime.combine(today, datetime.min.time())).count()})


@api.post("/reviews")
@login_required
def create_review(user):
    data = request.get_json(silent=True) or {}; word_id = str(data.get("wordId", "")); rating = int(data.get("rating", 0))
    ...
    if not card:
        if not Word.query.filter_by(id=word_id).first(): return jsonify({"error": "word_not_found"}), 404
        card = UserWordCard(user_id=user.id, word_id=word_id, first_learned_at=datetime.utcnow()); db.session.add(card); db.session.flush()
    reviewed_at = datetime.utcnow(); log = ReviewLog(user_id=user.id, word_id=word_id, card_id=card.id, rating=rating, review_type=str(data.get("reviewType", "review")), reviewed_at=reviewed_at, elapsed_seconds=data.get("elapsedSeconds"))
    ...
    card.state = "relearning" if rating == 1 else ("learning" if card.review_count == 1 else "review")
    ...
    card.due_at = reviewed_at + timedelta(days=intervals[rating]); db.session.add(log); db.session.commit()
...
def create_vocabulary(user):
    ...
    vocabulary = Vocabulary(name=name, owner_user_id=user.id, kind="user", priority=50, description=str(data.get("description", "")))
