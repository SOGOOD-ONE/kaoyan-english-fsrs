...
function render(root: HTMLElement, current: NewQueueWord | null, completed: number, total: number, submitting: boolean, message = "") {
  if (!current) {
  ...
  root.innerHTML = `<main class="study-page"><header class="study-session-head"><div><a href="/" class="study-back">← 返回首页</a><h1>学习新词</h1></div><div class="study-session-count">${completed} / ${total}</div></header><div class="study-session-track"><span style="width:${percent}%"></span></div><section class="study-word-panel new-learning-panel"><div class="study-word-meta">${escapeHtml(current.category || "核心词")} ${current.type ? `· ${escapeHtml(current.type)}` : ""}</div><div class="new-learning-prompt">${escapeHtml(prompt)}</div><div class="new-learning-options">${options.map(option => `<button class="new-learning-option" data-action="new-answer" data-option="${encodeURIComponent(option)}" ${submitting ? "disabled" : ""}>${escapeHtml(option)}</button>`).join("")}</div><button class="study-known-button" data-action="known" ${submitting ? "disabled" : ""}>斩</button>${message ? `<div class="new-learning-message">${escapeHtml(message)}</div>` : ""}</section></main>`;
}

declare global {
  interface Window { __NEW_WORDS__?: NewQueueWord[]; }
}
  ...
  try {
    ...
    const pickNext = (): NewQueueWord | null => {
      ...
      const waiting = scheduled
        .filter(entry => entry.word.id !== lastId && !entry.word.card.newComplete)
        .sort((a, b) => a.dueStep - b.dueStep);
    ...
    const rerender = (message = "") => render(root, current, completed, total, submitting, message);
    rerender();
    ...
    root.addEventListener("click", async event => {
      ...
      if (action === "known") {
        ...
        } catch { submitting = false; rerender("暂时无法保存，请重试。"); }
        return;
      }
