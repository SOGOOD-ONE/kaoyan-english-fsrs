...
async function renderHistoryPanel() {
  ...
  panel.innerHTML = `<div class="empty"><p>正在读取学习历史…</p></div>`;
  try {
    const history = await getStudyHistory(30);
    const recent = history.days.slice().reverse();
    const accuracy = history.totalReviews ? Math.round(recent.reduce((sum, day) => sum + day.accuracy * day.totalReviews, 0) / Math.max(1, recent.reduce((sum, day) => sum + day.totalReviews, 0))) : 0;
...
function renderCurrentCard(remaining?: number, total?: number) {
  ...
  const options = preview(current.card), w = current.word; const progress = remaining !== undefined && total !== undefined ? `<div class="session-progress"><span>本轮进度</span><strong>${Math.min(sessionAnswered + 1, total)} / ${total}</strong></div>` : "";
