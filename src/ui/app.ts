...
async function submitRating(rating: Grade) { if (!current || !answerVisible) return; const word = current.word; const localReview = await review(word, rating); try { await api(`/reviews`, { method: "POST", body: JSON.stringify({ wordId: word.id, rating, reviewType: mode, reviewId: localReview.reviewId, reviewedAt: localReview.reviewedAt }) }); } catch { }
