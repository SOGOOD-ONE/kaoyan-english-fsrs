...
function newDirection(card: NewCard): Direction {
  return card.newEcCorrect >= 2 ? "ce" : "ec";
}
...
async function loadItems(mode: Mode): Promise<{ items: Item[]; total: number }> {
  if (mode === "new") {
    throw new Error("new mode is handled separately");
  }
  if (mode === "mandatory") {
...
async function stopSession(sessionId: string) {
  try {
    await fetch(`/api/study/session/${encodeURIComponent(sessionId)}/stop`, { method: "POST", credentials: "include", keepalive: true });
  } catch {
    // Browser shutdowns can abort the request.
  }
}
...
function renderStudyShell(root: HTMLElement, mode: Mode, item: Item | undefined, index: number, total: number, answerVisible: boolean) {
  ...
  root.innerHTML = `<main class="study-page"><header class="study-session-head"><div><a href="/" class="study-back">← 返回首页</a><h1>${title}</h1><p>${desc}</p></div><div class="study-session-count" id="study-session-count">${Math.min(index + 1, total)} / ${total}</div></header><div class="study-session-track"><span id="study-session-progress" style="width:${percent}%"></span></div>${item ? `<section class="study-word-panel"><div class="study-word-meta" id="study-word-meta">${escapeHtml(item.word.category || "核心词")} ${item.word.type ? `· ${escapeHtml(item.word.type)}` : ""}</div><h2 id="study-word">${escapeHtml(item.word.word)}</h2><button class="study-reveal" id="study-reveal">${answerVisible ? "收起释义" : "显示释义"}</button><button class="study-known-button" data-action="known">认识，划掉</button><div id="study-answer-slot">${answerVisible ? `<div class="study-answer"><div class="study-meaning">${escapeHtml(item.word.meaning)}</div><div class="study-source">${escapeHtml(item.word.source || "")}</div></div><div class="study-ratings"><button data-action="rating" data-rating="1"><strong>不认识</strong><small>Again</small></button><button data-action="rating" data-rating="2"><strong>模糊</strong><small>Hard</small></button><button data-action="rating" data-rating="3"><strong>认识</strong><small>Good</small></button><button data-action="rating" data-rating="4"><strong>很熟</strong><small>Easy</small></button></div>` : ""}</div></section>` : `<section class="study-complete"><div class="study-complete-kicker">本次学习完成</div><h2>${title}完成</h2><p>本次完成 ${total} 个单词。</p><a href="/" class="study-home-btn">返回首页</a></section>`}<div class="study-footnote">已学习 <span id="study-count">${Math.min(index, total)}</span> 个 · 本次进度会同步到你的账号</div></main>`;
}
...
export async function mountStudy(root: HTMLElement, mode: Mode) {
  ...
  try {
    ...
    if (mode === "new") {
      ...
      let index = 0;
      let correctStage = 0;
      let session = await apiRequest<{ sessionId: string }>("/study/session/start", { method: "POST", body: JSON.stringify({ mode: "new" }) });
      ...
      const renderNew = (message = "") => {
        ...
        correctStage = card.newEcCorrect + card.newCeCorrect;
        ...
        const progressText = `${Math.min(correctStage + 1, 3)}/3 · ${direction === "ec" ? "英 → 汉" : "汉 → 英"}`;
        const percent = Math.min(100, Math.round((index / total) * 100));
        root.innerHTML = `<main class="study-page"><header class="study-session-head"><div><a href="/" class="study-back">← 返回首页</a><h1>学习新词</h1><p>2 次英译汉 + 1 次汉译英，正确完成 3 次后才进入 FSRS</p></div><div class="study-session-count">${index + 1} / ${total}</div></header><div class="study-session-track"><span style="width:${percent}%"></span></div><section class="study-word-panel new-learning-panel"><div class="study-word-meta">${escapeHtml(current.category || "核心词")} ${current.type ? `· ${escapeHtml(current.type)}` : ""}</div><div class="new-learning-stage">${progressText}</div><h2 class="new-learning-question">${escapeHtml(question)}</h2><div class="new-learning-prompt">${escapeHtml(prompt)}</div><div class="new-learning-options">${options.map(option => `<button class="new-learning-option" data-action="new-answer" data-option="${escapeHtml(option)}">${escapeHtml(option)}</button>`).join("")}</div><button class="study-known-button" data-action="known">认识，划掉</button>${message ? `<div class="new-learning-message">${escapeHtml(message)}</div>` : ""}</section><div class="study-footnote">今日已完成 ${queue.completed} 个 · 当前单词进度 ${correctStage}/3</div></main>`;
      };
      ...
      root.addEventListener("click", async event => {
        ...
        if (action === "new-answer") {
          ...
          const card = current.card || { newEcCorrect: 0, newCeCorrect: 0, knownExcluded: false, newComplete: false };
          const direction = newDirection(card);
          const selected = actionElement.dataset.option || "";
          const correctAnswer = direction === "ec" ? current.meaning : current.word;
          const correct = selected === correctAnswer;
          actionElement.classList.add(correct ? "correct" : "wrong");
          try {
            const result = await apiRequest<{ completed: boolean; card: NewCard }>("/study/new-answer", {
              method: "POST",
              body: JSON.stringify({ wordId: current.id, direction, correct }),
            });
            current.card = result.card;
            if (!correct) {
              window.setTimeout(() => renderNew("再试一次"), 350);
              return;
            }
            if (!result.completed) {
              renderNew();
