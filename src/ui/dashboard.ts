...
function stat(label: string, value: number) { return `<div class="overview-stat"><strong>${value.toLocaleString()}</strong><span>${label}</span></div>`; }
function formatDate(date = new Date()) { return `${date.getFullYear()}/${date.getMonth() + 1}/${date.getDate()}`; }

async function loadDashboard() {
  const [overview, time, today, me] = await Promise.all([
    apiRequest<Overview>("/study/overview"),
    apiRequest<StudyTime>("/study/time"),
    apiRequest<TodayProgress>("/study/today/progress"),
    apiRequest<{ user: User }>("/auth/me"),
  ]);
  return { overview, time, today, user: me.user };
}
...
export async function mountDashboard(root: HTMLElement) {
  ...
  try {
    const { overview, time, today, user } = await loadDashboard();
        ...
        <main>
          <section class="dashboard-progress-card">
            ...
            <div class="overview-stats">
              ${stat("总词数", overview.totalWords)}${stat("已背", overview.learnedWords)}${stat("已复习", overview.reviewedWords)}${stat("已掌握", overview.masteredWords)}${stat("剩余", overview.remainingWords)}
            </div>
          </section>
          <section class="dashboard-time-grid"><div class="dashboard-time-card"><span>今日学习时长</span><strong>${formatDuration(time.todaySeconds)}</strong></div><div class="dashboard-time-card"><span>总学习时长</span><strong>${formatDuration(time.totalSeconds)}</strong></div></section>
          <section class="dashboard-actions">
            <a class="study-action" href="/study/review"><span class="study-action-title">开始复习</span><span class="study-action-desc">${reviewRequired ? `今日还有 ${Math.max(0, today.mandatoryTotal - today.mandatoryCompleted)} 项必做复习` : "今日必做复习已经完成"}</span><span class="study-action-arrow">→</span></a>
